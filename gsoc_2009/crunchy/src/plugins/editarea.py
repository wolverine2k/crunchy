...
def enable_editarea(page, elem, textarea_id):  # tested
    ...
    data = textarea_id, config[page.username]['editarea_language']
    page.add_js_code(editAreaLoader_js % data)
...
def add_hidden_load_and_save(page, elem, textarea_id):  # tested
    ...
    hidden_load.attrib['class'] = u'load_python'
    ...
    hidden_save.attrib['class'] = u'save_python'
...
def add_load_python(page, parent, hidden_load_id, textarea_id):
    ...
    '''
    js_script = u"""path = file;
                    load_python_file('%s')""" % textarea_id
    insert_file_browser(page, parent,
                        hidden_load_id,
                        '/jquery_file_tree_all',
                        _('Select a file to open'),
                        js_script,
                        "load_python")

    onclick = u"c=getElementById('%s');c.style.visibility='hidden';c.style.zIndex=-1;"
    onclick = onclick % hidden_load_id
    btn = SubElement(parent, 'button', onclick=onclick)
    btn.text = _("Cancel")
    return
...
def add_save_python(page, parent, hidden_save_id, textarea_id):
    '''Inserts the widget required to browse for and save a local Python
       file.  This is intended to be used to load a file in the editor.
    '''
    input_id = u"input_" + hidden_save_id
    js_saved_script = "save_python_file(document.getElementById('%s').value, '%s');"
    js_saved_script = js_saved_script % (input_id, textarea_id)

    SubElement(parent, "br")
    SubElement(parent, 'input', name=u'url', size=u'60', id=input_id)
    SubElement(parent, "br")
    btn = SubElement(parent, 'button', onclick=js_saved_script)
    btn.text = _("Save file")

    js_script = u"""document.getElementById('%s').value=file;""" % input_id
    insert_file_browser(page, parent,
                        hidden_save_id,
                        '/jquery_file_tree_all',
                        _('Select a file to save'),
                        js_script,
                        "save_python")

    onclick = "c=getElementById('%s');c.style.visibility='hidden';c.style.zIndex=-1;"
    onclick = onclick % hidden_save_id
    btn = SubElement(parent, 'button', onclick=onclick)
    btn.text = _("Cancel")
    return

editArea_load_and_save = u"""
