...
//
// Modified for Crunchy/crst2s5.py by André Roberge - tested with Firefox on Mac OS
...
function toggle() {
	...
	if (!slides.disabled) {
		...
		s5mode = true;
		//fontScale(); // changed for Crunchy
                fontSize('1.7em')  // added for Crunchy
		for (var n = 0; n < smax; n++) {
			var slide = slideColl[n];
...
/*  Removed for Crunchy as it only caused problems...
...
function startup() {
	...
	externalLinks();
        // changed for Crunchy
	//fontScale();
        toggle();  // toggle twice to make the font change correctly...
        toggle();
        // end of change for Crunchy
	if (!isOp) notOperaFix();
	slideJump();
