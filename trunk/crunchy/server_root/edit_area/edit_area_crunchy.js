 function EAL(){this.version="0.7.2.2";
date=new Date();
...
},init_ie_textarea:function(id){var t=document.getElementById(id);
try{if(t&&typeof(t.focused)=="undefined"){t.focus();
t.focused=true;
t.selectionStart=t.selectionEnd=0;
get_IE_selection(t);
eAL.add_event(t,"focus",IE_textarea_focus);
eAL.add_event(t,"blur",IE_textarea_blur);
}}catch(ex){}},init:function(settings){if(!settings["id"])this.has_error();
...
},start:function(id){if(this.win!="loaded"){setTimeout("eAL.start('"+id+"');",50);
return;
}for(var i in eAL.waiting_loading){if(eAL.waiting_loading[i]!="loaded"&&typeof(eAL.waiting_loading[i])!="function"){setTimeout("eAL.start('"+id+"');",50);
return;
...
},toggle_off:function(id){if(window.frames["frame_"+id]){var frame=window.frames["frame_"+id];
...
}},toggle_on:function(id){if(window.frames["frame_"+id]){var frame=window.frames["frame_"+id];
...
elem.use_last=true;
eAL.start(id);
}},set_editarea_size_from_textarea:function(id,frame){var elem=document.getElementById(id);
...
},set_base_url:function(){if (!this.baseURL){var elements=document.getElementsByTagName('script');
for (var i=0;
i<elements.length;
...
},get_button_html:function(id,img,exec,isFileSpecific,baseURL){if(!baseURL)baseURL=this.baseURL;
...
return html;
},get_control_html:function(button_name,lang){for (var i=0;
...
},get_word_translation:function(val,lang){for(var i in eAL.lang[lang]){if(i==val)return eAL.lang[lang][i];
}return "_"+val;
},load_script:function(url){if (this.loadedFiles[url])return;
...
}},remove_event:function(obj,name,handler){if (obj.detachEvent)obj.detachEvent("on"+name,handler);
else obj.removeEventListener(name,handler,false);
},reset:function(e){var formObj=eAL.nav['isIE'] ? window.event.srcElement:e.target;
...
},submit:function(e){var formObj=eAL.nav['isIE'] ? window.event.srcElement:e.target;
if(formObj.tagName!='FORM')formObj=formObj.form;
for(var i in eAs){var is_child=false;
for (var x=0;
x<formObj.elements.length;
...
}},getSelectionRange:function(id){var sel={"start":0,"end":0};
...
},setSelectionRange:function(id,new_start,new_end){if(window.frames["frame_"+id]&&eAs[id]["displayed"]==true){window.frames["frame_"+id].editArea.area_select(new_start,new_end-new_start);
...
},setSelectedText:function(id,new_val){new_val=new_val.replace(/\r/g,"");
...
}},insertTags:function(id,open_tag,close_tag){var old_sel=this.getSelectionRange(id);
...
},hide:function(id){if(document.getElementById(id)&&!this.hidden[id]){this.hidden[id]=new Object();
...
}},show:function(id){if((elem=document.getElementById(id))&&this.hidden[id]){elem.style.display="inline";
...
},setFileEditedMode:function(id,file_id,to){var reg1=new RegExp('\\\\','g');
