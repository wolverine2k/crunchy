...
from pygments3.filter import apply_filters, Filter
from pygments3.filters import get_filter_by_name
from pygments3.token import Error, Text, Other, _TokenType
from pygments3.util import get_bool_opt, get_int_opt, get_list_opt, \
     make_analysator
...
class Lexer(object, metaclass=LexerMeta):
    ...
    def __repr__(self):
        if self.options:
            return '<pygments3.lexers.%s with %r>' % (self.__class__.__name__,
                                                     self.options)
        else:
            return '<pygments3.lexers.%s>' % self.__class__.__name__
