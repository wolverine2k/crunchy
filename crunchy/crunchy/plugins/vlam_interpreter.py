...
from crunchy.interface import config, plugin, translate, tostring, Element
...
def borg_javascript(page):
    ...
    function init_BorgInterpreter(uid){
        code = "import crunchy.interpreter\nborg=crunchy.interpreter.BorgConsole(group='%s',username='%s')";
        code += "\nborg.interact()\n";
...
def single_javascript(page):
    ...
    function init_SingleInterpreter(uid){
        code = "import crunchy.interpreter\nisolated=crunchy.interpreter.SingleConsole(username='%s')";
...
def type_info_javascript(page):
    ...
    function init_TypeInfoConsole(uid){
        code = "import crunchy.interpreter\nborg=crunchy.interpreter.TypeInfoConsole(group='%s', username='%s')";
        code += "\nborg.interact(ps1='<t>>> ')\n";
        var j = new XMLHttpRequest();
        j.open("POST", "/exec%s?uid="+uid, false);
        j.send(code);
...
##    code = "import crunchy.interpreter\n";
##    code += "isolated=crunchy.interpreter.SingleConsole()\n";
