...
from crunchy.interface import config, plugin, python_version, SubElement, Element
...
def get_tutorial_path(page):
    ...
    if page.is_local:   # tutorial loaded from browser - full path is known
        ...
        tutorial_fullpath = os.path.join(config['crunchy_base_dir'], "data", "server_root", page.url[1:])
    else:
        raise Exception
