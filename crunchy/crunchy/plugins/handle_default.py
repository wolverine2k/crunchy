...
root_path = join(config['crunchy_base_dir'], "data", "server_root")
...
def path_to_filedata(path, root, username=None):
    ...
    elif extension == "css" and crunchy.interface.last_local_base_url is not None:
        npath = normpath(join(crunchy.interface.last_local_base_url, normpath(path[1:])))
    ...
    try:
        ...
        if extension == 'css':
            try:
                ...
                crunchy.interface.last_local_base_url = None
        else:
