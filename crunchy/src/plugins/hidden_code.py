...
def register():
    ...
    plugin['register_tag_handler']("pre", "title", "hidden_code_external",
                                          hidden_code_external_widget_callback)
    plugin['register_http_handler'](
                         "/hidden_code%s" % plugin['session_random_id'],
                                       hidden_code_runner_callback)
    plugin['register_service']("reconstitute_hidden_file", reconstitute_hidden_file)


def hidden_code_runner_callback(request):
    """Handles execution of hidden_code samples as a thread. The request object will contain
    ...
    # add back code here
    code = reconstitute_hidden_file(request)
...
def reconstitute_hidden_file(request):
    code = request.data
    _lines = code.split("\n")
    i = 0
    for j, _line in enumerate(_lines):
        if _line == HIDDEN_CODE_MARKER:
            _lines[j] = config["extracted_lines"][request.args["uid"]][i]
            i += 1
    code = "\n".join(_lines)
    return code
    ...
    for _line in _lines:
        if hide:
            if _line.strip() == HIDDEN_CODE_END:
                hide = False
                config["extracted_lines"][uid].append('\n'.join(hidden_lines))
            else:
...
def hidden_code_external_widget_callback(page, elem, uid):
    """Handles embedding suitable code into the page in order to display and
    run code samples"""
    vlam = elem.attrib["title"]
    config["extracted_lines"][uid] = []
    ...
    btn2.attrib["onclick"] = "exec_hidden_code_externally('%s')" % uid
...
function exec_hidden_code_externally(uid){
    code=editAreaLoader.getValue("code_"+uid);
    if (code == undefined) {
