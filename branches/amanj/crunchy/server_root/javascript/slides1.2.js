// S5 v1.2a slides.js (modified for Crunchy)-- released into the Public Domain
...
// so that Python code can be typed in the various interactive elements.
//
...
function createNotesWindow() { // creates a window for our notes
	if (!s5NotesWindow || s5NotesWindow.closed) { // Create the window if it doesn't exist
		...
		// Note: Safari has a tendency to ignore window options
                ...
                //Crunchy notes: we open the notes from the server root
		s5NotesWindow = window.open('javascript/s5-notes.html', 's5NotesWindow', 'top=0,left=0');
	}
