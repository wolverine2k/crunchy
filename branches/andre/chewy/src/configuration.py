...
class UserPreferences(Borg):
    ...
    def __init__(self, root_dir=None):
        if not self.__initialised:
            ...
            if not os.path.exists(self.user_dir):  # first time ever
                self.create_path()
            self.user_file = os.path.join(self.user_dir, "chewy.cfg")
            self.load()
        return

    def create_path(self):
        '''Creates a path for chewy when none exists.'''
        try:
    ...
    def set_language(self, lang):
        ...
        if self._language == "fr":
            self.home = "src/html/chewy_index_fr.html"
            self.options = "src/html/options_fr.html"
        else: # English is the default
            self.home = "src/html/chewy_index.html"
