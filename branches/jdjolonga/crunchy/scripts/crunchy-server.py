...
import crunchy.interface
REQUIRED = 2.4
if crunchy.interface.python_version < REQUIRED:
...
crunchy.interface.plugin['session_random_id'] = str(int(random.random()*1000000000)) + str(
                                           int(random.random()*1000000000))

if crunchy.interface.python_version < 3:
...
from crunchy import account_manager
...
def run_crunchy(host='127.0.0.1', port=None, url=None):
    ...
    * open a web browser at given url, or a default if not specified
    ...
    import crunchy.configuration
    import crunchy.http_serve as http_serve
    import crunchy.pluginloader as pluginloader
...
def parse_options():
    ...
    #if options.debug:
    #    crunchy.interface.debug_flag = True
    #else:
    #    crunchy.interface.debug_flag = False
    #if options.debug_all:
    #    crunchy.interface.debug_flag = True
    #    for key in crunchy.interface.debug:
    #        crunchy.interface.debug[key] = True
    url = None
    crunchy.interface.completely_safe_url = None
    if options.url:
        url = convert_url(options.url)
    #if options.safe_url:
    #    crunchy.interface.completely_safe_url = urlsplit(options.safe_url)[1]
    #    url = convert_url(options.safe_url)
    #elif options.interactive:
    #    crunchy.interface.interactive = True
    #    url = convert_url(options.interactive)
    #if options.automated:
    #    crunchy.interface.config['initial_security_set'] = True
    #    crunchy.interface.config['automated'] = True
    port = None
    if options.port:
        port = options.port
    if options.accounts_file:
        if os.path.exists(options.accounts_file):
            crunchy.interface.accounts = account_manager.Accounts(
                                                          options.accounts_file)
        else:
            print("Specifid account file does not exists.")
            raise SystemExit
    elif options.single_user:
        crunchy.interface.accounts = account_manager.Accounts(False)
    else:
        crunchy.interface.accounts = account_manager.Accounts()
        if crunchy.interface.accounts == {}:  # can happen with empty password file
            crunchy.interface.accounts = account_manager.Accounts(False)
    return url, port

def convert_url(url):
    '''converts a url into a form used by Crunchy'''
    if crunchy.interface.interactive:
