...
import crunchy.CrunchyPlugin as CrunchyPlugin
import crunchy.interface
if crunchy.interface.python_version < 2.5:
    def all(S):
        for x in S:
            if not x:
               return False
...
def require_digest_access_authenticate(func):
    ...
    accounts = crunchy.interface.accounts
...
first_request = True
if "Unknown User" in crunchy.interface.accounts:
    crunchy.interface.unknown_user_name = "Unknown User"
...
elif crunchy.interface.accounts:
...
class HTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if first_request and bypass_authentication:
            self.crunchy_username = crunchy.interface.unknown_user_name
            first_request = False
        elif bypass_authentication:
            if ("Cookie" in self.headers and
                    crunchy.interface.plugin['session_random_id']
