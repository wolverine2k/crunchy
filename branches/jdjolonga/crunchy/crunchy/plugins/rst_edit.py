...
from crunchy.interface import plugin, SubElement, python_version, translate
_ = translate['_']
import crunchy.interface
from crunchy.plugins.editarea import editArea_load_and_save, insert_file_browser
import crunchy.plugins.rst_directives
...
def rst_edit_setup(page, elem, uid):
    ...
    if page.is_local:
        crunchy.interface.path_info['source_base_dir'] = page.url
    else:
        crunchy.interface.path_info['source_base_dir'] = os.path.normpath(os.path.join(
                        plugin['crunchy_base_dir'](), "server_root", page.url[1:]))
    base_dir = os.path.dirname(crunchy.interface.path_info['source_base_dir'])
