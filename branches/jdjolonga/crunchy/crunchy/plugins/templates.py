...
import os
import traceback
from crunchy.vlam import BasePage

_templates = {}

from crunchy.interface import config, plugin, u_print, Element, SubElement
...
def return_template(page, elem):
    ...
    if page.is_from_root:
        url = os.path.normpath(os.path.join(config['crunchy_base_dir'], "data", "server_root",
                                        base_dir[1:], url))
    elif page.is_local:
